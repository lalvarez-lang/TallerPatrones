...
internal class Program
{
    private static void Main(string[] args)
    {
        ...
        Command createCommand = new CreateCommand(task1);
        invoker.SetCommand(createCommand);
        invoker.ExecuteCommand();

        Command editCommand = new EditCommand(task1);
        invoker.SetCommand(editCommand);
        invoker.ExecuteCommand();

        Command deleteCommand = new DeleteCommand(task1);
        ...
        Command createCommand2 = new CreateCommand(task2);
        invoker.SetCommand(createCommand2);
        invoker.ExecuteCommand();

        Command editCommand2 = new EditCommand(task2);
        invoker.SetCommand(editCommand2);
        invoker.ExecuteCommand();

        Command completeCommand = new CompleteCommand(task2);
